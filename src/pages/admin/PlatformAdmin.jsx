      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <span style={{
            ...
            {payment.type}{payment.plan && ` — ${payment.plan}`}
          </span>
        </div>
        ...
        <span style={{
          ...
          borderRadius: 999, background: status.bg, color: status.color, flex: 1
        }}>
          {status.label}
        </span>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              ['Submitted',      payment.submittedAt
                ? new Date(payment.submittedAt).toLocaleString('en-GH') : '—'],
          ...
          {payment.status === 'cancelled' && (
            <div style={{
              ...
              background: '#F3F4F6', borderRadius: 'var(--radius-md)',
              fontSize: 'var(--text-sm)', color: '#374151'
            }}>
              <XCircle size={15} color="#6B7280" />
              This payment was cancelled by the church. No action required.
            </div>
          )}
          ...
          <div className="modal modal-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-actions" style={{ marginTop: 'var(--space-4)' }}>
                <button className="btn-outline" onClick={() => setShowReject(false)}>
                  Cancel
                </button>
                <button
                  className="btn-danger"
    ...
    <>
      ...
      >
        ...
        <p style={{
          ...
          fontWeight: 500, flex: 1, textTransform: 'capitalize'
        }}>
        ...
        <p style={{
          ...
          {org.subscriptionExpires
            ? new Date(org.subscriptionExpires).toLocaleDateString('en-GH', {
                day: 'numeric', month: 'short', year: 'numeric'
              })
            : '—'}
        </p>
        ...
        <div style={{
          ...
          {org.subscriptionStatus !== 'suspended' ? (
            <button
              ...
              style={{
                display: 'flex', alignItems: 'center', gap: 'var(--space-2)',
                fontSize: 'var(--text-xs)', padding: '0.5rem var(--space-3)'
              }}
...
// ── NOTIFICATION BELL ─────────────────────────
function NotificationBell({ token, onPaymentClick }) {
  ...
  const handleNotificationClick = (n) => {
    ...
    if (n.type === 'payment_submitted') onPaymentClick()
    setOpen(false)
  }
              ...
              notifications.map(n => (
                ...
                >
                  ...
                  <div style={{
                    ...
                    flexShrink: 0,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    background: n.type === 'payment_submitted'
...
export default function PlatformAdmin() {
  ...
  const churchStats = {
    ...
    active:    churches.filter(c => c.subscriptionStatus === 'active').length,
    trial:     churches.filter(c => c.subscriptionStatus === 'trial').length,
    expired:   churches.filter(c => c.subscriptionStatus === 'expired').length,
    suspended: churches.filter(c => c.subscriptionStatus === 'suspended').length,
  }
  const pendingPayments = payments.filter(p => p.status === 'pending_approval').length

  const TABS = [
    { id: 'overview', label: 'Overview'  },
    { id: 'payments', label: `Payments${pendingPayments > 0 ? ` (${pendingPayments})` : ''}` },
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
          ...
          <div>
            <p style={{
              fontFamily: 'var(--font-heading)', fontWeight: 800,
              color: 'var(--text-primary)', fontSize: 'var(--text-base)', lineHeight: 1.2
            }}>
              MinistryOS Admin
            </p>
            <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
              EM Control IT Solutions
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
          ...
          {pendingPayments > 0 && (
            <div style={{
              ...
              <Clock size={12} /> {pendingPayments} pending approval
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
            ...
            <div style={{
              ...
              <StatCard
                ...
                value={churchStats.total}
                sub={`${churchStats.active} active`}
                icon={Building2}
                color="var(--primary)"
              />
              <StatCard
              ...
              <StatCard
                ...
                sub="Awaiting review"
                icon={Clock}
                color={pendingPayments > 0 ? 'var(--warning)' : 'var(--text-muted)'}
              />
            </div>

            {/* Church status + recent approvals */}
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  {[
                    { label: 'Active',    count: churchStats.active,    color: 'var(--success)' },
                    { label: 'Trial',     count: churchStats.trial,     color: 'var(--info)'    },
                    { label: 'Expired',   count: churchStats.expired,   color: 'var(--warning)' },
                    { label: 'Suspended', count: churchStats.suspended, color: 'var(--danger)'  },
                    ...
                    <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                      ...
                      <span style={{
                        fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', flex: 1
                      }}>
                        {label}
                      </span>
                      <span style={{
                      ...
                      <div style={{
                        ...
                        <div style={{
                          height: '100%',
                          width: `${churchStats.total > 0
                            ? (count / churchStats.total) * 100 : 0}%`,
                    ...
                    {overview.recentApprovals.slice(0, 5).map(p => (
                      ...
                      >
                        <div>
                          ...
                          <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>
                            {p.reference} · {timeAgo(p.reviewedAt)}
                          </p>
                        </div>
        ...
        {activeTab === 'churches' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap' }}>
              ...
              <select
                value={churchStatus}
                onChange={e => setChurchStatus(e.target.value)}
                className="filter-select"
              >
                <option value="">All Statuses</option>
